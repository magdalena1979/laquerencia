...
{ href: '/servicios', label: 'Servicios', dropdown: [
      ...
      >
        <Menu isOpen={svc.isOpen} isLazy>
          <MenuButton
            ...
            color="#A8743F"
            _hover={{ color: '#15322D' }}
            borderBottom={location.pathname.startsWith(i.href) ? '2px solid #A8743F' : '2px solid transparent'}
            pb="2px"
            transition="all 0.2s ease"
          >
            {i.label}
          ...
          <MenuList bg="transparent" border="0" boxShadow="none" p={0} mt={2}>
            {i.dropdown.map((sub) => (
              <MenuItem
                key={sub.href}
                as={RouterLink}
